...
elseif strcmp(tag,'mn23_293a')  % Homer (C311)
    focal = 2;
    ttagon = datenum('20-Oct-2023 00:35:00');   % tag-on time
    ...
    tegood = NaN;                               % Hours into recording when good depth data ends
    pos_tagon = [70.30302 20.95817];            % lat/lon of tag-on
    pos_tagrecov = [70.2165 20.6379];           % lat/lon of tag recovery
...
elseif strcmp(tag,'mn23_299a')  % Elmo (C317)
    focal = NaN;
    ttagon = datenum('26-Oct-2023 07:02:00');   % tag-on time
    ttagoff = datenum('26-Oct-2023 17:47:32');  % tag-off time
    tstart = datenum('26-Oct-2023 06:54:56');   % reported recording start time
    tbas = NaN;                                 % baseline period start time
    tson = [NaN; NaN];                          % actual sonar CEE start/stop times
    tsgood = 0;                                 % Hours into recording when good depth data starts
    tegood = NaN;                               % Hours into recording when good depth data ends
    pos_tagon = [70.44802, 21.66422];           % lat/lon of tag-on
    pos_tagrecov = [70.38802, 21.25402];        % lat/lon of tag recovery
...
elseif strcmp(tag,'oo23_299a')  % Marge (C330)
    ...
    tbas = datenum('26-Oct-2023 00:30:00');     % baseline period start time (from Bridge log)
...
elseif strcmp(tag,'oo23_301a')  % Elmo (C317)
    focal = NaN;
    ttagon = datenum('28-Oct-2023 17:11:00');   % tag-on time
    ttagoff = datenum('29-Oct-2023 05:12:42');  % tag-off time
    tstart = datenum('28-Oct-2023 17:11:20');   % reported recording start time
    tbas = NaN;                                 % baseline period start time
    tson = [NaN; NaN];                          % actual sonar CEE start/stop times
    tsgood = 0;                                 % Hours into recording when good depth data starts
    tegood = NaN;                               % Hours into recording when good depth data ends
    pos_tagon = [70.5012, 21.3784];             % lat/lon of tag-on
    pos_tagrecov = [70.48013, 21.42272];        % lat/lon of tag recovery

elseif strcmp(tag,'mn23_301a')  % Simba (C329)
    focal = NaN;
    ttagon = datenum('28-Oct-2023 19:55:00');   % tag-on time
    ttagoff = datenum('30-Oct-2023 00:15:26');  % tag-off time
    tstart = datenum('28-Oct-2023 19:55:22');   % reported recording start time
    tbas = NaN;                                 % baseline period start time
    tson = [NaN; NaN];                          % actual sonar CEE start/stop times
    tsgood = 0;                                 % Hours into recording when good depth data starts
    tegood = NaN;                               % Hours into recording when good depth data ends
    pos_tagon = [70.4995, 21.2910];             % lat/lon of tag-on
    pos_tagrecov = [70.5911, 21.6438];          % lat/lon of tag recovery
...
elseif  strcmp(tag,'mn23_302a')  % Marge (C330)
    focal = NaN;
    ttagon = datenum('29-Oct-2023 02:44:00');   % tag-on time
    ttagoff = datenum('30-Oct-2023 03:04:40');  % tag-off time
    tstart = datenum('29-Oct-2023 02:44:02');   % reported recording start time
    tbas = NaN;                                 % baseline period start time
    tson = [NaN; NaN];                          % actual sonar CEE start/stop times
    tsgood = 0;                                 % Hours into recording when good depth data starts
    tegood = NaN;                               % Hours into recording when good depth data ends
    pos_tagon = [70.50345, 21.48357];           % lat/lon of tag-on
    pos_tagrecov = [70.5665, 21.5402];          % lat/lon of tag recovery
