...
timestamp_text = text(wlimits(2),wlimits(4),'','FontSize',14,'HorizontalAlignment', 'right')
...
% score video
if ~isfile(fullfile(out_dir,[tag,'.csv']))
    writetable(array2table(zeros(0,5),'VariableNames',{'w_vessel_start','w_vessel_end','decision point','Final_loop_start' 'Final_loop end'}),fullfile(out_dir,[tag,'.csv']))
end
winopen(fullfile(out_dir,[tag,'.csv'])) % tell excel to open the frame annotations file
